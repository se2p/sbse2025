...
For our output neurons, we have to choose an appropriate activation function that matches the given task. Later, we will try to solve a binary decision problem in which a robot can move to the right or left. 

Hence, we could use a sigmoid function, whose output is treated as a probability for choosing one of the two classes. However, due to the neuroevolution algorithm we will implement, we add two output neurons to the output layer, assign one output neuron to each class, and choose the class whose output neuron has the highest activation value.
...
```python
def gen_simple_network(inputs: int, hidden:int, outputs: int) -> Network:
    input_neurons = [InputNeuron(f"I{x}") for x in range(inputs)]
    hidden_neurons = [HiddenNeuron(f"H{x}") for x in range(hidden)]
    output_neurons = [OutputNeuron(f"O{x}") for x in range(outputs)]
...
net = gen_simple_network(inputs=5, hidden=3, outputs=2)
```
...
```python
gen_simple_network(inputs=3, hidden=3, outputs=3).show()
```
...
```python
gen_simple_network(5,3,5).activate([0, 1])
```
...
def evaluate_agent(network: Network, random_start=False) -> int:
    ...
    while steps < MAX_STEPS:
        ...
        # Activate the network, interpret the output, and advance the state.
...
```
    ...
    --------------------------------------------------------------------------------
    Parent 1 	| [83, 171, 149, 232, 186] 	| [-0.77, 0.28, -0.42, -0.79, -0.32]
    Parent 2 	| [236, 61, 159, 76, 187] 	| [0.27, -0.64, -0.58, -0.78, 0.37]
    Child 1 	| [83, 171, 159, 76, 187] 	| [-0.77, 0.28, -0.58, -0.78, 0.37]
    Child 2 	| [236, 61, 149, 232, 186] 	| [0.27, -0.64, -0.42, -0.79, -0.32]
...
```
    ...
    Neuron 	|		 Labels 		|		 Weights
    -------------------------------------------------------------------------------------
    Parent 	|	 [53, 141, 45, 152, 82] 	|	 [-0.21, 0.92, -0.65, -0.77, -0.12]
    Mutant 	|	 [52, 145, 44, 151, 86] 	|	 [-0.33, 0.88, -0.58, -0.84, 0.0]
...
```python
fitness_values, _ = solve_pole_sane(random_starts=False, print_log=False)
...
```
    ...
    SANE found solution in generation 7 after 1539 evaluations



    
![png](10/output_57_1.png)
...
```python
def plot(configs_data: dict[str, list[int]]):
    max_generation = 0
    
    # Find the maximum generation across all configs
    for fitness_values in configs_data.values():
        max_gen = max([len(f) for f in fitness_values])
        max_generation = max(max_generation, max_gen)
    
    # Plot each configuration
    for config_name, fitness_values in configs_data.items():
        # Pad fitness values
        ...
        plt.plot(x_range, mean_fitness_time, label=config_name)
...
```python
repetitions = 10
fitness_det, _ = evaluate_sane(False, repetitions)
fitness_stochastic, _ = evaluate_sane(True, repetitions)
data = {
    "Deterministic": fitness_det,
    "Stochastic": fitness_stochastic
}
plot(data)
```

    SANE found solution in generation 17 after 3526 evaluations
    SANE found solution in generation 11 after 2266 evaluations
    SANE found solution in generation 5 after 1025 evaluations
    SANE found solution in generation 20 after 4172 evaluations
    SANE found solution in generation 24 after 4876 evaluations
    SANE found solution in generation 5 after 1119 evaluations
    SANE found solution in generation 2 after 588 evaluations
    SANE found solution in generation 22 after 4426 evaluations
    SANE found solution in generation 25 after 5004 evaluations
    SANE found solution in generation 13 after 2693 evaluations
    SANE found solution in generation 30 after 6010 evaluations
    SANE found solution in generation 8 after 1649 evaluations
    SANE found solution in generation 125 after 25006 evaluations
    SANE found solution in generation 19 after 3819 evaluations
    SANE found solution in generation 1 after 222 evaluations
    SANE found solution in generation 1 after 304 evaluations
    SANE found solution in generation 76 after 15206 evaluations
    SANE found solution in generation 10 after 2053 evaluations
    SANE found solution in generation 3 after 666 evaluations
    SANE found solution in generation 76 after 15214 evaluations



    
![png](10/output_61_1.png)
...
```
    ...
    [[0.7605096452799558, 0.6948433837299102],
     [-0.36790294784635735, 0.24419120099462677],
     [-0.16711523069765355, -0.5645386625437954],
     [0.8747583523379714, -0.480563103847804]]
...
print(f"Networks vs. Subpopulation Size: {len(networks)} ==> {len(weights)}")
print(f"Connections vs. Subpopulations: {len(networks[0].connections)} ==> {len(weights[0])}")
...
```
    ...
    Networks vs. Subpopulation Size: 10 ==> 10
    Connections vs. Subpopulations: 3 ==> 3
    
    Connection Weights vs. Weight Values:
    [-0.6302610096558363, -0.5375498526123379, 0.6397894672408493]
    [-0.6302610096558363, -0.5375498526123379, 0.6397894672408493]
...
```
    ...
    [[0.3681979016440249, 0.9493117742495283, -0.8623567212669787]]
    [[0.3681979016440249, 0.9493117742495283, -0.8623567212669787]]
...
```
    ...
    [-0.978451895901117, -0.29085654502350367, 0.8845150305064944]
    [-0.978451895901117, -0.29085654502350367, 0.8845150305064944]
...
print(f"Child 1: {[float(w.weight) for w in network_to_weights(child_1)]}")
print(f"Child 2: {[float(w.weight) for w in network_to_weights(child_2)]}")
```

    Parent 1: [0.6347532545473333, 0.9799306687046727, 0.8500535806843941]
    Parent 2: [0.26782290949901966, 0.04128804851885404, -0.9923220084882582]
    Child 1: [0.6347532545473333, 0.04128804851885404, 0.8500535806843941]
    Child 2: [0.26782290949901966, 0.9799306687046727, -0.9923220084882582]
...
```
    ...
    Parent: [[-0.7913419126862131, -0.2774059372110742, -0.016126189766328514]]
    Mutant: [-0.7913419126862131, -0.2774059372110742, -0.016126189766328514]
...
```
    ...
    Before: [-0.3451084502778601, -0.3415523498153896, 0.7324252046042434, 0.24725374008182088, 0.9015899075417768]
    After: [0.9015899075417768, -0.3451084502778601, -0.3415523498153896, 0.7324252046042434, 0.24725374008182088]
...
```python
fitness_values, _ = solve_pole_cosyne(random_starts=True, print_log=False)
x_range = np.arange(0, len(fitness_values), 1)
...
```
    ...
    COSYNE found solution in generation 13 after 2659 evaluations
...
```python
def evaluate_NE_algorithms(random_start: bool, repetitions: int) -> (list[float], list[int], list[float], list[int]):
    fitness_SANE = []
    fitness_COSYNE = []
    evaluations_SANE = []
    evaluations_COSYNE = []
...
def plot_NE_algorithms(fitness_SANE: list[float], fitness_COSYNE: list[float]):
    # Padding
    plt.yscale("log")
...
```
    ...
    SANE found solution in generation 53 after 10608 evaluations
    COSYNE found solution in generation 24 after 4801 evaluations
    SANE found solution in generation 52 after 10474 evaluations
    COSYNE found solution in generation 21 after 4393 evaluations
    SANE found solution in generation 6 after 1212 evaluations
    COSYNE found solution in generation 6 after 1228 evaluations
    SANE found solution in generation 29 after 5817 evaluations
    COSYNE found solution in generation 3 after 674 evaluations
    SANE found solution in generation 59 after 11821 evaluations
    COSYNE found solution in generation 1 after 218 evaluations
    SANE found solution in generation 22 after 4445 evaluations
    COSYNE found solution in generation 37 after 7401 evaluations
    SANE found solution in generation 44 after 8829 evaluations
    COSYNE found solution in generation 35 after 7060 evaluations
    SANE found solution in generation 18 after 3626 evaluations
    COSYNE found solution in generation 5 after 1098 evaluations
    SANE found solution in generation 12 after 2420 evaluations
    COSYNE found solution in generation 5 after 1187 evaluations
    SANE found solution in generation 21 after 4247 evaluations
    COSYNE found solution in generation 18 after 3610 evaluations
    Average number of evaluations SANE: 6349.9
    Average number of evaluations COSYNE: 3167



    
![png](10/output_98_1.png)
